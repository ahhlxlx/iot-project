...
# ─────────────────────────────────────────────
#  PENDING ROUTE PREFERENCES
#  Queues ROUTE_PREF for BLE-only nodes that
#  currently have no WiFi IP. When the node
#  reconnects to WiFi (sender_ip changes from
#  "BLE-direct"/"BLE-only" to a real IP), the
#  pending pref is auto-delivered via UDP.
# ─────────────────────────────────────────────
pending_route_prefs = {}   # { node_id: { mode, weights, queued_at } }


def send_route_pref_to_node(node_id, mode, weights, node_ip):
    """
    ...
    cmd = {
        "type":           "ROUTE_PREF",
        "node_id":        "NODE_SV",      # passes valid_node_id() in node.py
        "target":         node_id,
        "mode":           mode,
    ...
    try:
        ...
        s.settimeout(2.0)
        s.sendto(json.dumps(cmd).encode(), (node_ip, MESH_PORT))
        s.close()
...
def deliver_pending_route_pref(node_id, node_ip):
    """
    Check if this node has a pending ROUTE_PREF and deliver it
    now that we have a WiFi IP for it.
    """
    pref = pending_route_prefs.pop(node_id, None)
    if pref:
        ok, msg = send_route_pref_to_node(node_id, pref["mode"], pref["weights"], node_ip)
        if ok:
            log.info(f"[RoutePref] Delivered pending {pref['mode']} to {node_id} "
                     f"(was queued at {pref['queued_at']})")
        else:
            log.warning(f"[RoutePref] Failed to deliver pending pref to {node_id}: {msg}")
        return ok, msg
    return False, "No pending pref"
...
def process_metric_packet(pkt, sender_ip):
    ...
    with matrix_lock:
        ...
        # ── Detect BLE→WiFi transition: deliver pending ROUTE_PREF ──
        old_ip = existing.get("sender_ip", "")
        is_ble_only_ip = old_ip in ("BLE-direct", "BLE-only", "")
        ...
        node_data = {
            ...
            "neighbours"      : nb,
            "routing_table"   : rt,
            "route_mode"      : pkt.get("route_mode", existing.get("route_mode", "balanced")),
            "weights"         : pkt.get("weights", existing.get("weights")),
...
# ─────────────────────────────────────────────
#  ROUTE PREFERENCE RELAY
#  server.py calls this to change a node's
#  routing weights. Gateway handles delivery:
#    WiFi node  → send UDP directly
#    BLE node   → queue, deliver when WiFi connects
# ─────────────────────────────────────────────
ROUTE_PREF_PROFILES = {
...
def api_route_pref():
    """
    Send route preference to one or more nodes.
    Body: { "node_ids": ["NODE_Ad", "NODE_gw"], "mode": "latency" }
      OR: { "node_id": "NODE_Ad", "mode": "latency" }  (single node)
    
    For WiFi-reachable nodes: sends UDP immediately.
    For BLE-only nodes: queues the pref and delivers it
    automatically when the node reconnects to WiFi.
    """
    body = flask_request.get_json(force=True)
    mode = body.get("mode", "balanced")

    if mode not in ROUTE_PREF_PROFILES:
        return jsonify({"error": f"Invalid mode. Use: {list(ROUTE_PREF_PROFILES.keys())}"}), 400

    weights = body.get("weights") or ROUTE_PREF_PROFILES[mode]

    # Accept both single node_id and list of node_ids
    node_ids = body.get("node_ids", [])
    if not node_ids and body.get("node_id"):
        node_ids = [body["node_id"]]
    if not node_ids:
        return jsonify({"error": "node_id or node_ids required"}), 400

    results = {}
    for nid in node_ids:
        with matrix_lock:
            node_data = health_matrix.get(nid)

        if not node_data:
            results[nid] = {"ok": False, "method": "none",
                            "message": "Node not in health matrix"}
            continue
        ...
        if is_ble_only:
            ...
            pending_route_prefs[nid] = {
                "mode": mode,
                "weights": weights,
                "queued_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            }
            results[nid] = {
                "ok": True, "method": "queued",
                "message": f"BLE-only node — queued {mode} mode. "
                           f"Will auto-deliver when {nid} connects to WiFi."
            }
...
@flask_app.route("/pending_prefs", methods=["GET"])
def api_pending_prefs():
    """Check what ROUTE_PREFs are waiting for BLE-only nodes."""
    return jsonify(dict(pending_route_prefs))
