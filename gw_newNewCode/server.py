...
def analyze_hop(from_id, to_id, from_node, route_entry, weights):
    ...
    w_rssi = metrics.get("wifi_rssi") or from_node.get("rssi", -99)
    ...
    b_rssi = metrics.get("ble_rssi", -99)
...
def trace_path(src, dst, nodes, weights):
    ...
    Key logic:
    - If src has dst in its routing_table → follow next_hop chain
    - If src/dst is GATEWAY → direct link
    - If no direct route exists but BOTH nodes are in health matrix →
      route through GATEWAY (src → GW → dst) since the gateway can
      reach all nodes (WiFi or BLE-Direct)
    """
    if src == dst:
        return []
    ...
    while current != dst and len(hops) < 10:
        ...
        # ── Current node is GATEWAY → go directly to destination ──
        if current == "GATEWAY":
            nd = nodes.get(dst)
            if nd:
        ...
        # ── Destination is GATEWAY → direct link ──
        if dst == "GATEWAY":
        ...
        if route_entry:
            ...
            # ── No route in table — check if direct neighbour ──
            neighbours = from_node.get("neighbours", [])
            if dst in neighbours:
                hops.append(analyze_hop(current, dst, from_node, None, weights))
                break

            # ── No direct route → try routing through GATEWAY ──
            # Both nodes are online in health matrix, meaning the
            # gateway can reach both (one via WiFi, one via BLE etc).
            # Route: current → GATEWAY → dst
            dst_node = nodes.get(dst)
            if dst_node and "GATEWAY" not in visited:
                # Hop 1: current node → GATEWAY
                hops.append(analyze_hop(current, "GATEWAY", from_node, None, weights))
                # Hop 2: GATEWAY → destination node
...
async def api_path_apply(request: Request):
    ...
    # 2. Collect unique node IDs (exclude GATEWAY)
    node_ids = set()
    for h in hops:
        if h["from"] != "GATEWAY":
            node_ids.add(h["from"])
...
async def api_gateway_check():
    """Check what endpoints the gateway has available."""
    results = {}
    async with httpx.AsyncClient(timeout=5.0) as client:
